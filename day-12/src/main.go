...
func helperEnd(end Coordinate, lines []string) int {
	neighboring := [4]Coordinate{{-1, 0}, {0, -1}, {1, 0}, {0, 1}}

	count := 0
	queue := []Coordinate{end}
	visited := map[string]bool{fmt.Sprintf("%d,%d", end.r, end.c): true}
	...
	for len(queue) > 0 {
		for _, node := range queue {
			...
			for _, delta := range neighboring {
				...
				if inRange && !visited[neighborKey] && charValue(string(lines[curr.r][curr.c]))-charValue(string(lines[neighbor.r][neighbor.c])) <= 1 {
